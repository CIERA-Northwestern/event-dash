...
If the dashboard is currently live at [ciera-event](https://event-dash-vixn8rne3icaqbfqrfrrrm.streamlit.app/), you can use the dashboard without any additional effort.
...
Some options are only available in the `config.yml` file found in the `src` directory (`./src/config.yml` if you are in the root directory, i.e. [here](https://github.com/CIERA-Northwestern/event-dash/blob/main/src/config.yml)).
...
The raw data lives in [the `data/raw_data` folder](https://github.com/CIERA-Northwestern/event-dash/tree/main/data/raw_data).
...
1. Click on the green "Code" button on [the GitHub repository](https://github.com/CIERA-Northwestern/event-dash), near the top of the page.
2. Select "Download ZIP."
3. Extract the downloaded ZIP file.
4. Optional: Move the extracted folder (`event-dash`; referred to as the code's "root directory") to a more-permanent location.
...
```
git clone git@github.com:CIERA-Northwestern/event-dash.git`
```
...
Much of the Streamlit functionality is also encapsulated in utility functions inside the `event_dash_lib/` directory, particularly in `event_dash_lib/streamlit_utils.py`.
